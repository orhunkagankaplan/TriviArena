...
const TriviArena = () => {
      ...
      <div className="min-h-screen bg-slate-950 text-white p-6 pt-12">
        ...
        <div className="max-w-2xl mx-auto bg-slate-900/70 p-10 rounded-3xl border border-pink-500/50">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-bold mb-3">Opponent Address</label>
              <input type="text" value={challengeAddress} onChange={(e) => setChallengeAddress(e.target.value)}
                placeholder="0x... or name.base.eth"
                className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl focus:border-pink-500 focus:outline-none text-white" />
            </div>
            <button onClick={createChallenge} disabled={!stake || !category || !challengeAddress}
              className="w-full py-5 bg-gradient-to-r from-pink-600 to-rose-600 rounded-xl font-bold disabled:opacity-50">
              Generate Link
            </button>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
        <div className="max-w-2xl w-full">
          ...
          <h2 className="text-4xl font-black text-center mb-8">Challenge Created!</h2>
          <div className="bg-slate-900/70 p-8 rounded-3xl border border-green-500/50">
            <div className="bg-slate-800 p-4 rounded-xl mb-6 break-all font-mono text-sm">{challengeLink}</div>
            <button onClick={copyLink} className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl font-bold mb-4">
              {linkCopied ? <Check className="w-5 h-5 inline mr-2" /> : <Copy className="w-5 h-5 inline mr-2" />}
            ...
            <button onClick={resetGame} className="w-full py-3 bg-slate-800 rounded-xl">Back to Home</button>
          </div>
        </div>
      </div>
    );
  ...
  if (screen === 'quiz') {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white p-6 pt-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-6">
              ...
              {(mode === '1v1' || mode === 'challenge') && (
                <div className="flex gap-4 bg-slate-800/50 px-5 py-3 rounded-full border border-slate-700">
            ...
            <div className={`flex items-center gap-3 px-5 py-3 rounded-full ${timeLeft <= 5 ? 'bg-red-500/20 border-red-500/50' : 'bg-slate-800/50 border-slate-700'} border`}>
          ...
          <div className="bg-slate-900/70 backdrop-blur-2xl p-10 rounded-3xl border border-slate-800 mb-6">
            ...
            <div className="space-y-4">
              {question.options.map((option, index) => {
                ...
                if (answered) {
                  if (index === question.correct) bgClass = 'bg-gradient-to-r from-green-600 to-emerald-600 border-green-500';
                  else if (index === selectedAnswer) bgClass = 'bg-gradient-to-r from-red-600 to-rose-600 border-red-500';
                  else bgClass = 'bg-slate-800/50 border-slate-800 text-slate-500';
                }
                return (
                  <button key={index} onClick={() => handleAnswer(index)} disabled={answered}
                    className={`w-full p-6 rounded-2xl border-2 ${bgClass} text-left font-bold text-lg transition-all`}>
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-slate-700 flex items-center justify-center font-black">
                        {String.fromCharCode(65 + index)}
                      </div>
                      <span className="flex-1">{option}</span>
                      {answered && index === question.correct && <Check className="w-6 h-6" />}
                    </div>
                  </button>
                );
              })}
      ...
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
        <div className="max-w-3xl w-full">
          <div className="text-center mb-10">
            <div className={`inline-block p-6 rounded-full shadow-2xl mb-6 ${won && !tie ? 'bg-gradient-to-br from-green-500 to-emerald-500' : 'bg-gradient-to-br from-red-500 to-rose-500'}`}>
              {won && !tie ? <Trophy className="w-16 h-16 text-white" /> : <span className="text-6xl">💔</span>}
            </div>
            <h2 className="text-5xl font-black mb-4">{won && !tie ? 'VICTORY!' : tie ? "TIE!" : 'DEFEAT'}</h2>
            {(mode === '1v1' || mode === 'challenge') && (
              <div className={`text-4xl font-black ${won && !tie ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="bg-slate-900/70 backdrop-blur-2xl p-10 rounded-3xl border border-slate-800">
            <div className="grid grid-cols-2 gap-6 mb-8">
              ...
              {(mode === '1v1' || mode === 'challenge') ? (
                <div className="text-center p-6 bg-slate-800/50 rounded-2xl">
                  <div className="text-slate-400 text-sm font-bold mb-2">Opponent</div>
                ...
                <div className="text-center p-6 bg-slate-800/50 rounded-2xl">
                  ...
                  <div className="text-slate-500 text-sm mt-1">Correct</div>
                </div>
              )}
            </div>

            <div className="space-y-4">
              <button onClick={resetGame} className="w-full py-5 bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl font-bold text-lg">
                <Sparkles className="w-5 h-5 inline mr-2" />
                Play Again
              </button>
              <button onClick={resetGame} className="w-full py-4 bg-slate-800 hover:bg-slate-700 rounded-xl font-semibold">
                Back to Home
              </button>
            </div>
          </div>
